...
def load_h5_files_from_folder(folder_path):
    ...
    labels = [get_fileseed(x) for x in h5_files]
    ...
    for idx, h5_file in enumerate(h5_files):
        ...
        if "pd" in h5_file:
            labels[idx] = f"{labels[idx]}_pd"
            if "arctan" in h5_file:
                labels[idx] = f"{labels[idx]}_arctan"
    return labels, dataframes
...
labels, dataframes = load_h5_files_from_folder(folder_path)

def roc_curve_compare_no_weights(list_test, list_test_pd, list_pred, list_labels, name, title):
    true = list_test["CLASSIFICATIONS"]["EVENT"]["class"][()]
    proba_class = [
        pred["CLASSIFICATIONS"]["EVENT"]["class"][:, 1][()] for pred in list_pred
    ]
    print(len(proba_class))
    score = [roc_auc_score(true, proba_class[i]) for i in range(len(proba_class))]
    print(score)
    print("max", max(score))
    print("min", min(score))
    fpr, tpr, thresholds = zip(
        *[roc_curve(true, proba_class[i]) for i in range(len(proba_class))]
    )
    fig = plt.figure()
...
def roc_curve_compare_weights(
    list_test, list_test_pd, list_pred, list_labels, name, title, compare=False
):
    print(list_labels)
    true = list_test["CLASSIFICATIONS"]["EVENT"]["class"][()]
    true_pd = list_test_pd["CLASSIFICATIONS"]["EVENT"]["class"][()]
    proba_class = [
        pred["CLASSIFICATIONS"]["EVENT"]["class"][:, 1][()] for pred in list_pred
    ]
    print(len(proba_class))
    ...
    print(len(true))
    print([len(x) for x in proba_class])
    print(len(weights))
    ...
    roc_curve_list = []
    for i in range(len(proba_class)):
        if 'pd' in str(list_labels[i]):
            roc_curve_list.append(roc_curve(true_pd, proba_class[i], sample_weight=weights))
        else:
            roc_curve_list.append(roc_curve(true, proba_class[i], sample_weight=weights))
...
# roc_curve_compare_no_weights(df_true,dataframes ,labels, "seed_comparison_4b_QCD_dnn", "Comparision of 4b-QCD trainings with dnn vars as inputs using different seeds")
# roc_curve_compare_weights(df_true,dataframes ,labels, "seed_comparison_4b_QCD_dnn", "Comparision of 4b-QCD trainings with dnn vars as inputs using different seeds")

if args.weights:
    roc_curve_compare_weights(df_true, df_true_pd, dataframes, labels, args.name, args.title)
else:
    roc_curve_compare_no_weights(df_true, df_true_pd, dataframes, labels, args.name, args.title)
