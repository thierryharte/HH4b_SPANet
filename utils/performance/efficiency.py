...
from efficiency_functions_2 import *
...
if args.input_spanet_pred:
    ...
    spanet_dir = "/eos/home-r/ramellar/2_spanet_predictions/out_prediction_files/"
    ...
    spanet_dict = {
        ...
        # "5_jets_ATLAS_ptreg_allklambda_train_klinput_newkl_newCuts_oldCutsEval_noklinp": f"{spanet_dir}spanet_prediction_nc_noklinp_on_oc_kl3p5.h5",
        # "5_jets_ATLAS_ptreg_allklambda_train_klinput_newkl_newCuts_newCutsEval_noklinp": f"{spanet_dir}spanet_prediction_nc_noklinp_on_nc.h5",
        # "5_jets_ATLAS_ptreg_allklambda_train_klinput_newkl_newCuts_newCutsEval_noklinp_fm": f"{spanet_dir}spanet_prediction_nc_noklinp_on_nc_fm.h5",
        # "5_jets_ATLAS_ptreg_allklambda_train_klinput_newkl_newCuts_oldCutsEval_noklinp_fm": f"{spanet_dir}spanet_prediction_nc_noklinp_on_oc_kl3p5_fm.h5",
        ...
        #
        "4_jets_first":  f"{spanet_dir}out_0_spanet_prediction_4jets.h5",
        "5_jets_first": f"{spanet_dir}out_1_spanet_prediction_5jets.h5",
...
if args.input_true:
    ...
    true_dir = "/eos/home-r/ramellar/2_spanet_predictions/out_prediction_files/true_files/"
    ...
    true_dict = {
        ...
        # "5_jets_allklambda": f"{true_dir}output_JetGood_allkl_test.h5", 
        ...
        # "5_jets_allklambda_newkl_oldCuts": f"{true_dir}output_JetGood_test_allkl_new_kl_oldcuts.h5",
    }
...
#This is done are for every input file we give
...
#idx_true gives the idx of the jets that come from the b quarks from the H decay. 
# If one event only has one Higgs, the truth labek of the other Higg's jet will be -999, so we look for events where bpth H1 and H2 have jets with inidces above or equal to 0
# Spanet always goves a prediction , so we need to do this from the truth file
mask_fully_matched = [ak.all(ak.all(idx >= 0, axis=-1), axis=-1) for idx in idx_true]
# This masks gives True or false, per event in each true file
...
# spanet_dict.keys() is the dictionnary with the name of the prediction files so if it contains "5 jets" it will retiurn 1, which corresponds to the "5 jets" test file
...
print("idx_spanet_pred_fully_matched", [len(idx) for idx in idx_spanet_pred_fully_matched])
print("(idx_spanet_pred)", [len(idx) for idx in (idx_spanet_pred)])


correctly_fully_matched_spanet = [
    (
        ak.all(
            #checks the name of the prediction file and selects the truth file for which we compare the inidces for H1 (or H2)
...
#This is an array of true and false


print("correctly_fully_matched_spanet",correctly_fully_matched_spanet)
...
#fraction of fully matched (number of trues in the mask over total nb)
...
for label, frac in zip(list(true_dict.keys()), frac_fully_matched_mask30):
    print(f"Fraction of fully matched events {label} (DeltaR>30): {frac:.4f}")
...
for label, eff in zip(list(true_dict.keys()), efficiency_fully_matched_run2_mask30):
    print(f"Efficiency fully matched  for Run 2 {label} (DeltaR>30): {eff:.4f}")
...
for i in range(len(total_efficiency_fully_matched_run2_mask30)):
    print(
        "Total efficiency fully matched for Run 2 {} (DeltaR>30): {:.4f}".format(
...
#same but applying the mask of 30GeV for the distance
