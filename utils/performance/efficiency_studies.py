...
from efficiency_functions import check_names, separate_klambda, plot_diff_eff, plot_diff_eff_klambda, plot_histos_1d, plot_histos_2d, plot_mhh, reco_higgs, distance_pt_func, best_reco_higgs
...
print(spanet_dict)
# bin definitions. Basically make a list of bin ranges for the different data types. Not sure why though.
#mh_bins = [
#    np.linspace(0, 300, n)
#    for n in [150, 150, 150, 40, 40, 40, 40, 40, 40, 150, 150, 150, 150, 150, 150, 150]
#]
#mh_bins_peak = [
...
# In case the list of bin numbers is not as long as the list of true data (to which it should match), extend by the bin range of the last entry.
...
mh_bins = np.linspace(0, 300, 75)
mh_bins_peak = np.linspace(90, 160, 35)
mh_bins_2d = np.linspace(0, 500, 100)
...
if args.klambda:
    ...
    # mh_bins += [np.linspace(0, 300, 150) for _ in range(len(kl_values_true))]
    ...
    print("mh_bins", len(mh_bins))
    print("mh_bins_peak", len(mh_bins_peak))
    print("mh_bins_2d", len(mh_bins_2d))
...
print("mask_fully_matched", [len(mask) for mask in mask_fully_matched])
print([f"{list(spanet_dict.keys())[i]}_{check_names(list(spanet_dict.keys())[i])}" for i in range(len(idx_spanet_pred))])
...
min_idx = [m[mask] for m, mask in zip(min_idx, mask_30)]
...
print(len(comb_idx))
print(len(min_idx[0]))
print(len(comb_idx[0]))

idx_run2_pred_fully_matched = [
    ak.Array(comb)[m]
...
if not args.data:
    ...
    correctly_fully_matched_run2 = [
        ...
        for i, i2 in zip(
            idx_true_fully_matched, idx_run2_pred_fully_matched
        )
    ]
    ...
    print("\n")
    for label, eff in zip(list(true_dict.keys()), efficiency_fully_matched_run2):
        print(f"Efficiency fully matched  for Run 2 {label} (DeltaR>30): {eff:.3f}")

    total_efficiency_fully_matched_run2 = [
        efficiency_fully_matched_run2[i] * frac_fully_matched[i]
        for i in range(len(efficiency_fully_matched_run2))
    ]
    print("\n")
    for i in range(len(total_efficiency_fully_matched_run2)):
        print(
            "Total efficiency fully matched for Run 2 {} (DeltaR>30): {:.3f}".format(
                list(true_dict.keys())[i],
                total_efficiency_fully_matched_run2[i],
            )
        )
    ...
    correctly_fully_matched_spanet = [
        (
            ak.all(
                idx_true_fully_matched[check_names(list(spanet_dict.keys())[i])][
                    :, 0
                ]
                == idx_spanet_pred_fully_matched[i][:, 0],
                axis=1,
    ...
    print("\n")
    for label, eff in zip(
        list(spanet_dict.keys()), efficiencies_fully_matched_spanet
    ):
    ...
    if args.klambda:
        ...
        efficiency_fully_matched_run2_allklambda = (
            efficiency_fully_matched_run2[-len(kl_values_true):]
        )
        total_efficiency_fully_matched_run2_allklambda = (
            total_efficiency_fully_matched_run2[-len(kl_values_true):]
        )
        print(run2_dataset)
        run2_idxs = [check_names(key) for key in true_dict.keys() if "f{run2_dataset}_" in key]
        first_run2_idx = (
            len(kl_values_true) // kl_values_true.count(kl_values_true[0]) * (-1) * 8
        )  # HERE
        last_run2_idx = (
        ...
        efficiencies_fully_matched_allklambda = (
            efficiencies_fully_matched_spanet_allklambda
            + efficiency_fully_matched_run2_allklambda[first_run2_idx:last_run2_idx]
        )
        total_efficiencies_fully_matched_allklambda = (
            total_efficiencies_fully_matched_spanet_allklambda
            + total_efficiency_fully_matched_run2_allklambda[first_run2_idx:last_run2_idx]
        )
        print("\n")
        print("Plotting efficiencies fully matched for all klambda values")
        plot_diff_eff_klambda(
            ...
            allkl_names_spanet + [run2_dataset],
            "eff_fully_matched_allklambda",
            plot_dir,
        )
        plot_diff_eff_klambda(
...
for index, (j, idx) in enumerate(zip(jet_fully_matched, idx_true_fully_matched)):
    best_reco_higgs(j, idx)
true_higgs_fully_matched = [
    best_reco_higgs(j, idx)
    for j, idx in zip(jet_fully_matched, idx_true_fully_matched)
]
...
if not args.data:
    ...
    for j in range(len(list(true_dict.keys()))):
        diff_eff_run2.append([])
        unc_diff_eff_run2.append([])
        total_diff_eff_run2.append([])
        unc_total_diff_eff_run2.append([])
        for i in range(1, len(mhh_bins)):
            mask = (true_hh_fully_matched[j].mass > mhh_bins[i - 1]) & (
                true_hh_fully_matched[j].mass < mhh_bins[i]
            )
            eff_run2 = ak.sum(correctly_fully_matched_run2[j][mask]) / ak.count(
                correctly_fully_matched_run2[j][mask]
            )
            unc_eff_run2 = sqrt(
                eff_run2
                * (1 - eff_run2)
                / ak.count(correctly_fully_matched_run2[j][mask])
            )
            ...
            diff_eff_run2[j].append(eff_run2)
            unc_diff_eff_run2[j].append(unc_eff_run2)
            total_diff_eff_run2[j].append(total_eff_run2)
            unc_total_diff_eff_run2[j].append(unc_total_eff_run2)
...
if not args.data:
    ...
    plot_diff_eff(
        ...
        (
            diff_eff_run2
            if not args.klambda
            else diff_eff_run2[: -len(kl_values_true)]
        ),
        (
            unc_diff_eff_run2
            if not args.klambda
            else unc_diff_eff_run2[: -len(kl_values_true)]
        ),
        true_dict,
        (
            diff_eff_spanet
    ...
    plot_diff_eff(
        ...
        (
            total_diff_eff_run2
            if not args.klambda
            else total_diff_eff_run2[: -len(kl_values_true)]
        ),
        (
            unc_total_diff_eff_run2
            if not args.klambda
            else unc_total_diff_eff_run2[: -len(kl_values_true)]
        ),
        true_dict,
        (
            total_diff_eff_spanet
...
run2_higgs_fully_matched = [
    best_reco_higgs(j, idx)
    for j, idx in zip(jet_fully_matched, idx_run2_pred_fully_matched)
]
...
for bins, name in zip([mh_bins, mh_bins_peak], ["", "_peak"]):
    for number in [1, 2]:
        plot_histos_1d(
            ...
            [run2[:, number - 1].mass for run2 in run2_higgs_fully_matched],
...
#    plot_histos_2d(
#        mh_bins_2d[check_names(label)],
#        run2_higgs_fully_matched[check_names(label)],
#        list(true_dict.keys())[check_names(label)],
#        "Run2",
#        plot_dir=plot_dir,
#    )
...
# mask_hh_mass_400 = [
#     (true_hh_fully_matched[i].mass > 400)
#     & (true_hh_fully_matched[i].mass < 700)
